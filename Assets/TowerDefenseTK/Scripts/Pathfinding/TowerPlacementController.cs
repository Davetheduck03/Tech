...
namespace TowerDefenseTK
{
    ...
    public class TowerPlacementController : MonoBehaviour
    {
        ...
        private bool ValidatePlacement(Vector2Int gridPos, Vector3 worldPos, out PlacementError error)
        {
            ...
            // Check 2: Tile type (using PathNodeGenerator if available)
            ...
            if (mapLoader != null && !mapLoader.CanBuildAt(worldPos))
            {
                error = PlacementError.TileNotBuildable;
                return false;
            }
        ...
        {
            // Instantiate tower
            // Note: TowerUnit initializes itself in Awake() using its serialized unitData field
            // The prefab should already have the correct TowerSO assigned
            ...
            if (PathNodeGenerator.Instance != null)
            {
                PathNodeGenerator.Instance.BlockNodeForTower(gridPos);
            }
        ...
        public void RemoveTower(GameObject towerObject, Vector2Int gridPos, int refundAmount = 0)
        {
            ...
            if (PathNodeGenerator.Instance != null)
            {
                PathNodeGenerator.Instance.UnblockNode(gridPos);
            }
